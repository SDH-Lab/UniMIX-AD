            ...
            if hasattr(mm, 'all_gates'):
                for i in range(len(mm.all_gates)):
                    ...
                    if i_loss is not None:
                        gate_losses.append(i_loss)
                    else:
                        print(f"[WARN] The gate loss if {mn}, modality: {i} is emtpy, check weather call <get_loss> twice.")
            
            # Collect MCG losses from TransformerEncoderLayer modules
            if hasattr(mm, 'mcg_losses') and len(mm.mcg_losses) > 0:
                mcg_losses.extend(mm.mcg_losses)

    # Aggregate gate losses into a single tensor
        if gate_losses:
            total_gate_loss = torch.stack(gate_losses).sum()  # Use `stack` and `sum`
            # Ensure 1D tensor for DataParallel compatibility
            if total_gate_loss.dim() == 0:
                total_gate_loss = total_gate_loss.unsqueeze(0)
        else:
            total_gate_loss = torch.zeros(1, device=x.device)  # Handle empty case - 1D tensor
        
        # Aggregate MCG losses
        if mcg_losses:
            total_mcg_loss = torch.stack(mcg_losses).mean()  # Average MCG losses
            # Ensure 1D tensor for DataParallel compatibility
            if total_mcg_loss.dim() == 0:
                total_mcg_loss = total_mcg_loss.unsqueeze(0)
        else:
            total_mcg_loss = torch.zeros(1, device=x.device)  # Handle empty case - 1D tensor
        
        return x, total_gate_loss, total_mcg_loss
...
class Custom3DCNN(nn.Module):
    ...
    def __init__(self, hidden_dim=128):
        ...
        self.fc = nn.Linear(hidden_dim * 4 * 4 * 4, hidden_dim)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_mask = None):
        ...
        if self.self_attn:
            ...
            if self.mlp_sparse:
                for i in range(len(chunk_size)):
                    output, mcg_loss = self.mlp(self.norm2(x[i]), self.expert_index)
                    x[i] = x[i] + self.dropout2(output)
                    if mcg_loss is not None:
                        mcg_losses.append(mcg_loss)
            else:
                for i in range(len(chunk_size)):
        ...
        # Store MCG losses for later retrieval
        self.mcg_losses = mcg_losses
        return x
